...
An enumeration data type (defined using `typedef`) specifies a set of constant (integer) values that are
all distinct. You can either set the constant values in the enumeration type yourself:
...
data types or arrays. But we can also have the fields of a structure to be
...
This is a structure that represents a single square in the maze. It has an attribute
`type` (using the enumeration type `SquareType` from above), 
and another attribute `visited` that lets us know whether we've visited this square before 
or not (useful for checking if your path has loops).
...
typedef struct Maze {
  ...
  MazePos start; //Starting position
  MazePos end; //Ending position
} Maze;
```
...
Note that we CANNOT do this in the other order! If we `free(matrix)` first, we
will have no way of getting the addresses of the row arrays we need to
free.

### 5. 2D Array Coordinates vs. Cartesian Coordinates
...
letter 'i'). 
In contrast, in2D arrays (and matrices), `[0][0]` is at the top left of the
grid (the letter 'a').
...
Assume that at each square, we try to first go north, then south, then east, then
west.
...
In this assignment, you only need to write **one function**, `depthFirstSolve` in
...
1. `Maze * m`: the maze you are trying to solve. `Maze` is declared in `maze.h`.
    ...
    declaration in `maze.h`; they explain in detail how the content 
    in the input file is read into `Maze`
...
1. `step` tells you how far along in the current path you are;
2. if you successfully exit the maze on the `n`th step, `path[n]` should be `\0`.
...
function. This assignment will probably have the highest ratio of *time spent thinking about what code to write* to *amount of code written* of any
...
path from input files, and call `checkPath` to see if it is right. There is a 
part of code in `hw9.c` (between `#ifdef CHECK_PATH` and `#endif` tag) that shows how to do this.
...
As before, we will check if there is memory leak. 1 byte of of memory leak means 1 point off.
Remember to test your HW on `ecegrid`, and use `valgrind` to check memory leak.
...
## Submission
You will only need to submit the following file(s):
1. `solver.c`
