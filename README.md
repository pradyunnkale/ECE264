...
* `integrator-hw4.c`: This file contains your integration functions: `integrate`, which takes the structure you defined in `hw4.h` and uses that information to perform a numerical integration, and `runIntegrate`, which takes the name of an input file and an output file as arguments and:
    ...
    2) Uses `integrate` to numerically integrate five functions (provided for you, called `func1`, `func2`, `func3`, `func4`, and `func5`). For accuracies, we recommend using [Trapezoidal Rule](https://en.wikipedia.org/wiki/Trapezoidal_rule#Uniform_grid) as the integration method. 
...
Please make sure to drag these files on Gradescope directly or compress them into a single .zip file, without wrapping them in an additional folder. Submissions that do not follow this instruction will not be graded by the auto-grader.